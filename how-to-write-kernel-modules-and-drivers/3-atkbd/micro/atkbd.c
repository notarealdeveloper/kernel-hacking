...
static int atkbd_connect(struct serio *serio, struct serio_driver *drv)
{
	...
	atkbd->ps2dev.serio    	= serio;	// safer: ps2_init(&atkbd->ps2dev, serio);
	serio->dev.driver_data 	= atkbd;	// safer: serio_set_drvdata(serio, atkbd);
	serio->drv 		= drv;		// safer: serio_open(serio, drv);
...
static void atkbd_disconnect(struct serio *serio)
{
	...
	serio->close(serio);				// this and next line were..
	serio->drv = NULL;				// serio_close(serio);
