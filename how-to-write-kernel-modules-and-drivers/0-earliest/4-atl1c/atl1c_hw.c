...
static bool atl1c_read_current_addr(struct atl1c_hw *hw, u8 *eth_addr)
{
	...
	*(u16 *) &eth_addr[0] = htons((u16)addr[1]);
	...
	/* I ADDED THIS */
	printk("Yo bitch, your MAC is: ");
	for (i = 0; i < 6; i++) {
		printk("%02x", eth_addr[i]);
		if   (i != 5) printk(":");
		else printk("\n");
	}

	/* I ADDED THIS */
	printk("Yo bitch, hw->mac_addr is: ");
	...
	printk("Yo bitch, hw->mac_addr is: ");
	for (i = 0; i < ETH_ALEN; i++) {
		printk("%02x", hw->mac_addr[i]);
		if   (i != 5) printk(":");
		else printk("\n");
	}

	/* I ADDED THIS */
	printk("Yo bitch, hw->perm_mac_addr is: ");
	for (i = 0; i < ETH_ALEN; i++) {
		printk("%02x", hw->perm_mac_addr[i]);
		if   (i != 5) printk(":");
		else printk("\n");
	}
...
static int atl1c_get_permanent_address(struct atl1c_hw *hw)
{
	...
	if (atl1c_read_current_addr(hw, hw->perm_mac_addr)){
		...
		//	hw->perm_mac_addr[j] = 0xff;
		//}
		hw->perm_mac_addr[0] = 0x08;
		return 0;
	}
